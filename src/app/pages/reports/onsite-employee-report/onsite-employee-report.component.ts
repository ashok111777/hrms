...
@Component({
  selector: 'app-onsite-employee-report',
  templateUrl: './onsite-employee-report.component.html',
  styleUrls: ['./onsite-employee-report.component.scss']
})

export class OnsiteEmployeeReportComponent implements OnInit {
  ...
  GetData(mode: string) {
    ...
    if (this.EmployeeData.valid) {
      this.restApi.getService('api/web/v1/onsite/report' + '?startDate=' + startDate + '&endDate=' + endDate).subscribe((resp: any) => {
  ...
  for (const employee of employeeList) {
    const onsiteDetailsRows = [];

    for (const onsiteDetail of employee.onsiteDetails) {
      const onsiteDetailRow = [
        onsiteDetail.id,
        onsiteDetail.onsiteLocation,
        onsiteDetail.countryName,
        onsiteDetail.productName,
        onsiteDetail.fromDate,
        onsiteDetail.toDate,
        onsiteDetail.description,
        onsiteDetail.purposeOfVisit,
      ];

      onsiteDetailsRows.push(onsiteDetailRow);
    }
      ...
      [
        {
          table: {
            body: [
              ['id', 'onsiteLocation', 'countryName', 'productName', 'fromDate', 'toDate', 'description', 'purposeOfVisit'],
              ...onsiteDetailsRows,
            ],
          },
        },
    ...
    const docDefinition: any = {
      ...
      content: [
        ...
        { text: 'Onsite Report', style: 'header' },
        {
          alignment: 'justify',
          columns: [
            {
        ...
        { 
            ...
            table: {
              body: [
                ['Id', 'Employeeid', 'Name', 'Team', 'Designation', 'Onsitedetails'],
                // Add the dynamically generated table body here
                ...tableBody,
              ],
            },
        },
