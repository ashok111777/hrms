...
@Component({
  selector: 'app-employeeexit-report',
  templateUrl: './employeeexit-report.component.html',
  styleUrls: ['./employeeexit-report.component.scss']
})
export class EmployeeexitReportComponent {
  ...
  GetData(mode: string) {
    ...
    if (this.EmployeeData.valid) {
      this.restApi.getService('api/web/v1/exit/report' + '?startDate=' + startDate + '&endDate=' + endDate).subscribe((resp: any) => {
  ...
  async generatePDF(employeeList: any[], startDate: any, endDate: any) {
    ...
    const docDefinition: any = {
      ...
      content: [
        ...
        { text: 'Employee Exit Report', style: 'header' },
        {
          alignment: 'justify',
          columns: [
            {
