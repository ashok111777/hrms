...
@Component({
  selector: 'app-latecome-report',
  templateUrl: './latecome-report.component.html',
  styleUrls: ['./latecome-report.component.scss']
})
export class LatecomeReportComponent implements OnInit {
  ...
  constructor(
    ...
    private sanitizer: DomSanitizer, private http: HttpClient, private datePipe: DatePipe, private snackBar: MatSnackBar
  ) { }
  ...
  GetData(mode: string) {
    ...
    if (this.EmployeeData.valid) {
      this.restApi.getService('api/web/v1/pdf/lateComeReport' + '?startDate=' + startDate + '&endDate=' + endDate).subscribe((resp: any) => {
  ...
  async generatePDF(employeeList: any[], _startDate: string, _endDate: string) {
    ...
    const headerName = ['Emp Id', 'Name', 'Date', 'In Time', 'Out Time', 'Total Working Hours', 'Late Hours', 'Early Hours', 'Extra Hours'];
    const tableHeader = headerName.map((item) => ({
      text: item,
      style: 'tableHeader',
    }));
    const tableData = [tableHeader];
    employeeList.forEach((employee) => {
      const rowData = item.map((item) => ({
        text: employee[item] !== null ? employee[item].toString() : '',
        style: 'tableData',
    ...
    const docDefinition: any = {
      ...
      content: [
        ...
        { text: 'LATE COME REPORT', style: 'header' },
        {
          alignment: 'justify',
          columns: [
            {
              text: 'Date: ' + this.datePipe.transform(this.startDate, 'dd/MM/yyyy') + ' - ' + this.datePipe.transform(this.endDate, 'dd/MM/yyyy'),
